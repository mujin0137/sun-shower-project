    ...
    const fetchCitiesWeather = async () => {
      try {
        const promises = majorCities.map((city) =>
          fetch(`${API_BASE_URL}/api/weather/current?city=${city.city}`)
            .then((res) => res.json())
            .then((data) => ({
              ...city,
    ...
    const fetchSelectedCityWeather = async () => {
      try {
        ...
        const currentRes = await fetch(
          `${API_BASE_URL}/api/weather/current?city=${selectedCity}`
        );
        ...
        const forecastRes = await fetch(
          `${API_BASE_URL}/api/weather/forecast?city=${selectedCity}`
        );
